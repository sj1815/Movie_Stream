...
var rankingCollection = database.OpenCollection("rankings", database.Connect())
...
func AdminReviewUpdate() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := c.ShouldBind(req); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Error occurred while binding the review data"})
			return
		}

		sentiment, rankVal, err := GetReviewRanking(req.AdminReview)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error occurred while getting review ranking: " + err.Error()})
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error occurred while updating the movie"})
			return
		}
...
func GetReviewRanking(adminReview string) (string, int, error) {
	...
	if OpenAiApiKey == "" {
		return "", 0, errors.New("OPENAI_API_KEY is not set in the environment")
	}
